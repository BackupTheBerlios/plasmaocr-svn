...
static void mark_hot_points(Chaincode *cc, unsigned char **pixels, int w, int h)
{
    int i;
    for (i = 0; i < cc->node_count; i++)
        pixels[(int) cc->nodes[i].y][(int) cc->nodes[i].x] = HOT_POINT;
}
...
void chaincode_print(Chaincode *cc)
{
    ...
    {
        int k;
        printf("    %d: (%f, %f) degree %d;", i, cc->nodes[i].x, cc->nodes[i].y, cc->nodes[i].degree);
...
Chaincode *chaincode_scale(Chaincode *cc, float coef)
{
    int i;
    Chaincode *result = MALLOC1(Chaincode);
    result->width  = cc->width  * coef;
    result->height = cc->height * coef;
    ...
    for (i = 0; i < result->node_count; i++)
    {
        ...
        result->nodes[i].x = (cc->nodes[i].x * coef);
        result->nodes[i].y = (cc->nodes[i].y * coef);
...
void chaincode_get_rope_middle_point(Chaincode *cc, int rope_index, float *px, float *py)
{
    ...
    if (n % 2)
    {
        x += .5 * chaincode_dx(s[i]);
        y += .5 * chaincode_dy(s[i]);
    }
    *px = x;
    *py = y;
}
...
Chaincode *chaincode_load(FILE *f)
{
    ...
    fread(&cc->width, 1, sizeof(cc->width), f);
    fread(&cc->height, 1, sizeof(cc->height), f);

    for (i = 0; i < n; i++)
    {
        fread(&cc->nodes[i].x, 1, sizeof(cc->nodes[i].x), f);
        fread(&cc->nodes[i].y, 1, sizeof(cc->nodes[i].y), f);
...
void chaincode_save(Chaincode *cc, FILE *f)
{
    ...
    write_int32(n, f);
    write_int32(r, f);
    fwrite(&cc->width, 1, sizeof(cc->width), f);
    fwrite(&cc->height, 1, sizeof(cc->height), f);

    for (i = 0; i < n; i++)
    {
        fwrite(&cc->nodes[i].x, 1, sizeof(cc->nodes[i].x), f);
        fwrite(&cc->nodes[i].y, 1, sizeof(cc->nodes[i].y), f);
...
void assert_nodes_do_not_repeat(Chaincode *cc)
{
    int i;
    for (i = 0; i < cc->node_count; i++)
    {
        int j;
        for (j = i + 1; j < cc->node_count; j++)
        {
            assert(cc->nodes[i].x != cc->nodes[j].x
                || cc->nodes[i].y != cc->nodes[j].y);
        }
    }    
}

void assert_chaincodes_equal(Chaincode *cc1, Chaincode *cc2)
{
    int i;
    
    assert(cc1->width == cc2->width && cc1->height == cc2->height);

    assert(cc1->node_count == cc2->node_count && cc1->rope_count == cc2->rope_count);
    
    for (i = 0; i < cc1->node_count; i++)
        assert(node_equal(&cc1->nodes[i], &cc2->nodes[i]));

    for (i = 0; i < cc1->rope_count; i++)
        assert(rope_equal(&cc1->ropes[i], &cc2->ropes[i]));
}
...
static void get_chaincode_and_render(unsigned char **framework, int w, int h, FilePair fp)
{
    ...
    assert_chaincodes_equal(cc, cc2);
